...
def _gate_filter(fourier_image, beta, gamma=1.5):
...
def _wiener_filter(fourier_image, beta, gamma=1.5):
    filt = np.abs(fourier_image) / \
        (gamma*beta+np.abs(fourier_image))
    return filt


def _calc_noise_profile(data, perc=50, win_size=12, beta_const=False,
...
def _get_filter(chunk_arr, fourier_image, betas, half_win, beta_const=False,
    ...
    if gate:
        filt = _gate_filter(fourier_image, noise_profile, gamma=gamma)
    else:
        filt = _wiener_filter(fourier_image, noise_profile, gamma=gamma)
...
def _run_filter(data, betas, win_size=12, beta_const=False,
                image_indepen_noise=False, gate=False,
                gamma=1.5, use_gpu=True):
    ...
    for count, (i, j) in enumerate(product(strides_x,
        ...
        filt = _get_filter(chunk_arr, fourier_image, betas,
...
def noise_gate(data, perc=50, use_gpu=True, kwargs_filter={}, kwargs_noise={}):
    ...
    perc: int
     ...
     default 50 (%) (median)
    kwargs_filter: dict
     dictionary of keywords for filter
        beta_const: boolean
         Set for single value of beta across Fourier space
         default false
        image_indepen_noise: boolean
        ...
        win_size : int
         length of sub-cube side
    kwargs_noise: dict
     dictionary of keywords for noise filtering
        gate: boolean
    ...
    # set defaults
    kwargs_filter['gamma'] = kwargs_filter.get('gamma', 1.5)
    kwargs_filter['gate'] = kwargs_filter.get('gate', False)

    kwargs_noise['beta_const'] = kwargs_noise.get('beta_const', False)
    kwargs_noise['image_indepen_noise'] = kwargs_noise.get('image_indepen_noise', False)
    kwargs_noise['win_size'] = kwargs_noise.get('win_size', 12)

    betas = _calc_noise_profile(data, perc=perc, **kwargs_noise)

    return _run_filter(data, betas, use_gpu=use_gpu,
